...
if(viewportWidth<720 && mobile != true){
  ...
  d3.select(".timeline-end").text("JUN")
...
// var blues = ["#bbbbbb","#cccccc","rgb(202, 222, 163)","rgb(126, 176, 161)","rgb(76, 127, 151)","rgb(25, 84, 115)"];
// var blues = ["#cccccc","#e0f0fd","#b0d6f6","#85bbe8","#5fa0d6","#307dbc"];
// var blues = ["#bbbbbb","hsl(62,100%,90%)","rgb(202, 222, 163)","rgb(126, 176, 161)","rgb(76, 127, 151)","rgb(25, 84, 115)"];
...
// var blues = ["rgb(247,251,255)","rgb(222,235,247)","rgb(198,219,239)","rgb(158,202,225)","rgb(66,146,198)","rgb(33,113,181)","rgb(8,81,156)","rgb(8,48,107)"];
// blueScale;

// var blueScale = d3.scale.pow().domain([0,300])
//     .range(["hsl(62,100%,90%)", "hsl(228,30%,20%)"])
//     .exponent(.1)
//     .interpolate(d3.interpolateHcl)
//     .clamp(true)
...
// rgb(139, 0, 0);
// rgb(255, 255, 224);
// var reds = ["#bbbbbb","#cccccc","rgb(244, 212, 137)","rgb(216, 136, 89)","rgb(182, 49, 50)","rgb(137, 0, 36)"];
// var reds = ["#cccccc","#fcdcdc","#f5aaaa","#e87d7d","#d65454","#b82c2c"];
// var reds = ["#cccccc","#fcdcdc","#f5aaaa","#e87d7d","#d65454","#b82c2c"];
...
// var blueScale = d3.scale.linear().domain([0,5,10,15,50,300]).range(blues).clamp(true);
var blueScale = d3.scale.threshold().domain([2,5,7,10,15,50,100,5000]).range(blues);

// var redScale = d3.scale.linear().domain([0,5,10,15,50,300]).range(reds).clamp(true);
...
queue()
  ...
  .defer(d3.csv, "dates.csv") //2KB
  .defer(d3.csv, "county_names_3.csv") //81KB
  // .defer(d3.csv, "views_county_4.csv") //4.8MB
  .defer(d3.csv, "video_rank_6.csv") //42KB
  .defer(d3.csv, "locations.csv") // 1KB
  .defer(d3.csv, "unique_video_titles.csv") //81KB
  .defer(d3.csv, "spark_data.csv") //29KB
  .defer(d3.csv, "first_load.csv") //29KB
  .await(ready);
...
function ready(error,
  ...
  function buildVideos(){
    ...
    if(allDatesSelected){
      ...
      if(uploader=="Trump"){
        thing = [{date:"",agg_views:"1062982",candidate:"Trump",video_shortened_id:"619"},{date:"",agg_views:"424355",candidate:"Trump",video_shortened_id:"1050"},{date:"",agg_views:"398227",candidate:"Trump",video_shortened_id:"504"}];
      }
      else if(uploader=="Clinton"){
        thing = [{date:"",agg_views:"440951",candidate:"Clinton",video_shortened_id:"1259"},{date:"",agg_views:"354553",candidate:"Clinton",video_shortened_id:"201"},{date:"",agg_views:"309785",candidate:"Clinton",video_shortened_id:"987"}];
      }
      else{
        thing = [{date:"",agg_views:"1062982",candidate:"Trump",video_shortened_id:"619"},{date:"",agg_views:"440951",candidate:"Clinton",video_shortened_id:"1259"},{date:"",agg_views:"424355",candidate:"Trump",video_shortened_id:"1050"}];
      }
  ...
  function secondLoadClean(){
    ...
    d3.csv("views_county_4.csv", function(countyViews) {
  ...
  var weekScale = d3.scale.quantize().domain([parseDate("20151028"),parseDate("20160531")]).range(d3.range(sparkLineCount));
  ...
  function buildSpark(){
    ...
    if(uploader!="Trump" && uploader!="Clinton"){
      ...
      sparkLines = d3.select(".view-spark")
        ...
        .style("height",function(d){
          if(uploader=="Clinton"){
            return heightScale(13*d.values[0].values/(321418820/10000)) + "px";
          }
          return heightScale(13*d.values[1].values/(321418820/10000)) + "px";
        })
        .style("visibility","hidden")
        .on("mouseover",function(d,i){
          ...
          sparkToolTip.style("left",i*6+"px").style("bottom",function(){
            if(uploader=="Clinton"){
              return 23 + heightScale(13*d.values[0].values/(321418820/10000)) + "px";
            }
            return 23 + heightScale(13*d.values[1].values/(321418820/10000)) + "px";
          })
          .style("visibility","visible")
          ;
    ...
    d3.select(".timeline-button").on("click",function(){
      if(!tickRunning){
        if(!loadStarted){
          ...
          tickRunning = true;
          d3.select(this).text("❚❚").style("line-height","11px");
          if(endDate == 216){
            startDate = 0;
            endDate = startDate + offSet;
          }
